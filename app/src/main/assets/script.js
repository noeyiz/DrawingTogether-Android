...
var putPoint = function (e) {
    if (dragging) {
        console.log(e.changedTouches[0].pageX, canvas.offsetLeft, e.changedTouches[0].pageY, canvas.offsetTop);
        ctx.lineTo(e.touches[0].clientX, e.touches[0].clientY);
        ctx.stroke();
        ctx.beginPath();
        ctx.arc(e.touches[0].clientX, e.touches[0].clientY, radius, 0, Math.PI * 2);
        ctx.fill();
        ctx.beginPath();
        ...
        var mouseX = parseInt(e.touches[0].clientX - offsetX);
        var mouseY = parseInt(e.touches[0].clientY - offsetY);
...
var btn = function (e) {
    try {
        ...
        img.src = srcG;
        AutoDrawInterface.setImage(srcG);
...
canvas.addEventListener("touchrout", disengage, false);
